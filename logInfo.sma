...
public taskPrintInfo(args[]) {
    ...
    server_print("[logInfo.amxx] Join: ^"%s^" (account ^"%s^") (address ^"%s^") joined the game" , name, steamId, ip);
    ColorChat(0, GREEN,  "[Holo]^x01玩家 ^x03%s ^x01[^x03%s^x01]加入了游戏", name, ip);
    new datadir[64];
    ...
    log_to_file(logInfoFilePath, "Join: ^"%s^" (account ^"%s^") (address ^"%s^") joined the game" , name, steamId, ip);
