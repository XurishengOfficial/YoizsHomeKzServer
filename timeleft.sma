...
public plugin_init()
{
	...
	/*
		set_task(Float:delay,const func[],id = 0,const args[]="",len = 0,const flags[]="", repeat = 0)
		1 delay 延时
		2 func 需要被调用的函数名
		3 定时任务的id?? 默认表示该定时任务的id由系统指定
		4 保存 向 func 传递的多个参数 的数组
		5 arg[] 的长度
		6 flags 默认空 

		set_task(0.1，"shin",id,_,_,"b")  		0.1秒无限执行shin事件，不需要执行次数，因为使用b _代表使用默认参数
		set_task(1.0，"shin",id,_,_,"a",10)		1秒执行一次shin事件，执行10次后不执行
		只执行一次shin事件
			set_task(1.0，"shin",id,_,_,"a",1)
			set_task(1.0，"shin",id)
...
{
	//如果参数 amx_time_voice 参数被设置为1 则执行
	if (get_cvar_num("amx_time_voice"))
	{
		...
		//调用说话的函数 只会说特定的单词?
		//05/20/2021 - 20:02:11
		//spk "fvox/time_is_now eight _period two pm "
...
{
	new temp[7][32]
	new secs = tmlf % 60	//剩余秒
	new mins = tmlf / 60	//剩余分
