...
public plugin_init()
{
	...
	readMapCycle2(g_mapCycle, g_nextMap, charsmax(g_nextMap))
...
public plugin_end() {
	...
	server_print("[NextMap.amxx] ========== Post: amx_map is %s ==========", mapName);
}
...
readMapCycle2(szFileName[], szNext[], iNext) {
	...
	if (file_exists(szFileName))
	{
		// 没有读取g_mapNum;
		if(!mapNum) {
			while (read_file(szFileName, idx++, szBuffer, charsmax(szBuffer), b))
			{
				if (!isalnum(szBuffer[0]) || !ValidMap(szBuffer)) continue
				++mapNum;
			}
		...
		new nextRandomIndex = random_num(1, mapNum);
		new actualIdx = 0;
		while (read_file(szFileName, idx++, szBuffer, charsmax(szBuffer), b))
		{
			if (!isalnum(szBuffer[0]) || !ValidMap(szBuffer)) continue
			...
			if(actualIdx == nextRandomIndex) {
				copy(szNext, iNext, szBuffer);
				server_print("[NextMap.amxx] ========== Pre: amx_nextmap is %s ==========", szBuffer);
				return;
			}
		}
	}
	if(!mapNum) server_print("Couldn't find a valid map or the mapcycle.txt doesn't exist");
	return;
}
