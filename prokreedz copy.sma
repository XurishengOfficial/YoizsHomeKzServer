...
public plugin_init()
{
	...
	RegisterHam(57, "player", "Ham_PlayerPreThink");
...
public Version(id)
{
	ColorChat(id, GREEN, "%s^1 Server use ProKZ ^3%s ^1, Last Edit ^3%s ^1by ^3Perfectslife.", prefix, VERSION, LAST_EDIT)
}
...
public kz_menu(id)
{
	...
	formatex(title, 285, "\w#KZ Server \dby Perfectslife ^n^n\dPresent time %s^nMap \y%s\d & Timeleft \y%d:%02d^n\dType map \y%s",thetime, MapName, tl/60, tl%60, mapinfo)
...
public AdminMenu(id)
{
	...
	menu_additem( menu, "AMXX管理员菜单", "1" )
	menu_additem( menu, "设置地图类型", "2" )
	menu_additem( menu, "发起地图投票", "3" )
	menu_additem( menu, "连跳板子设定\y(鼠标准星标记)", "4" )
	menu_additem( menu, "更新最新记录\y(XJ CC NT)", "5" )
	menu_additem( menu, hidespec, "6" )
